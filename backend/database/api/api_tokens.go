...
func (a *APITokensAPI) CreateAPIToken(ctx context.Context, userID int, req *models.APITokenRequest) (*models.APITokenResponse, error) {
	// Generate new token
	rawToken, tokenPrefix, err := tokens.GenerateAPIToken()
	if err != nil {
		return nil, fmt.Errorf("failed to generate token: %w", err)
	}
	
	// Hash token for storage
	tokenHash := tokens.HashAPIToken(rawToken)
...
func (a *APITokensAPI) ValidateAPIToken(ctx context.Context, rawToken string) (*models.User, error) {
	// Validate format
	if !tokens.ValidateAPITokenFormat(rawToken) {
	...
	// Check expiration
	if tokens.IsTokenExpired(expiresAt) {
