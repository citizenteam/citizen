...
type ProjectToml struct {
	...
	// Citizen section for port/domain configuration
	Citizen struct {
		Port   int    `toml:"port"`
...
func parseProjectToml(data []byte) (*ConfigPort, error) {
	...
	fmt.Printf("[TOML] Checking metadata.citizen.port: %d\n", config.Metadata.Citizen.Port)
	if config.Metadata.Citizen.Port != 0 {
		fmt.Printf("[TOML] ✅ Found port in metadata.citizen.port: %d\n", config.Metadata.Citizen.Port)
		return &ConfigPort{
			Port:   config.Metadata.Citizen.Port,
			Source: "project.toml (metadata.citizen.port)",
		}, nil
	}
	...
	fmt.Printf("[TOML] Checking citizen.port: %d\n", config.Citizen.Port)
	if config.Citizen.Port != 0 {
		fmt.Printf("[TOML] ✅ Found port in citizen.port: %d\n", config.Citizen.Port)
		return &ConfigPort{
			Port:   config.Citizen.Port,
			Source: "project.toml (citizen.port)",
		}, nil
	}
